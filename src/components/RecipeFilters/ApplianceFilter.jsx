...
export default function ApplianceFilter({ data = [], selectedAppliances = [], onAppliancesChange }) {
    ...
    const appliances = [
        ...new Set(
            (data || []).map(recipe => recipe.appliance?.toLowerCase()).filter(Boolean)
        )
    ];

    const filteredAppliances = appliances.filter(
        a => a.includes(value.toLowerCase()) && !selectedAppliances.includes(a)
    );

    const handleSelectAppliance = (appliance) => {
        if (!selectedAppliances.includes(appliance)) {
            onAppliancesChange([...selectedAppliances, appliance]);
        }
        setValue("");
        setOpen(false);
    };
    ...
    const handleRemoveAppliance = (appliance) => {
        onAppliancesChange(selectedAppliances.filter(a => a !== appliance));
    };
                    ...
                    <Dropdown>
                        ...
                        <List>
                            {filteredAppliances.map((appliance, idx) => (
                                <Item key={idx} onClick={() => handleSelectAppliance(appliance)}>
                                    {formatText(appliance)}
                                </Item>
                            ))}
                        </List>
                    </Dropdown>
            ...
            {selectedAppliances.length > 0 && (
                <Tags>
                    {selectedAppliances.map((appliance, idx) => (
                        <Tag key={idx}>
                            {formatText(appliance)}
                            <Remove onClick={() => handleRemoveAppliance(appliance)}>                <svg width="13" height="13" viewBox="0 0 13 13" fill="none" xmlns="http://www.w3.org/2000/svg">
